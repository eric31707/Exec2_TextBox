namespace Q3
{
	partial class Form1
	{
		/// <summary>
		/// 設計工具所需的變數。
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// 清除任何使用中的資源。
		/// </summary>
		/// <param name="disposing">如果應該處置受控資源則為 true，否則為 false。</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows Form 設計工具產生的程式碼

		/// <summary>
		/// 此為設計工具支援所需的方法 - 請勿使用程式碼編輯器修改
		/// 這個方法的內容。
		/// </summary>
		private void InitializeComponent()
		{
			this.inputtxt = new System.Windows.Forms.TextBox();
			this.comparebtn = new System.Windows.Forms.Button();
			this.resultlbl = new System.Windows.Forms.Label();
			this.label1 = new System.Windows.Forms.Label();
			this.SuspendLayout();
			// 
			// inputtxt
			// 
			this.inputtxt.Location = new System.Drawing.Point(239, 109);
			this.inputtxt.Name = "inputtxt";
			this.inputtxt.Size = new System.Drawing.Size(100, 22);
			this.inputtxt.TabIndex = 0;
			// 
			// comparebtn
			// 
			this.comparebtn.Location = new System.Drawing.Point(427, 109);
			this.comparebtn.Name = "comparebtn";
			this.comparebtn.Size = new System.Drawing.Size(75, 23);
			this.comparebtn.TabIndex = 1;
			this.comparebtn.Text = "比較";
			this.comparebtn.UseVisualStyleBackColor = true;
			this.comparebtn.Click += new System.EventHandler(this.comparebtn_Click);
			// 
			// resultlbl
			// 
			this.resultlbl.AutoSize = true;
			this.resultlbl.Location = new System.Drawing.Point(612, 114);
			this.resultlbl.Name = "resultlbl";
			this.resultlbl.Size = new System.Drawing.Size(42, 12);
			this.resultlbl.TabIndex = 2;
			this.resultlbl.Text = "resultlbl";
			// 
			// label1
			// 
			this.label1.AutoSize = true;
			this.label1.Location = new System.Drawing.Point(27, 114);
			this.label1.Name = "label1";
			this.label1.Size = new System.Drawing.Size(154, 12);
			this.label1.TabIndex = 3;
			this.label1.Text = "請輸入生日(例如:2022/10/30)";
			// 
			// Form1
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(6F, 12F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			this.ClientSize = new System.Drawing.Size(800, 450);
			this.Controls.Add(this.label1);
			this.Controls.Add(this.resultlbl);
			this.Controls.Add(this.comparebtn);
			this.Controls.Add(this.inputtxt);
			this.Name = "Form1";
			this.Text = "Form1";
			this.ResumeLayout(false);
			this.PerformLayout();

		}

		#endregion

		private System.Windows.Forms.TextBox inputtxt;
		private System.Windows.Forms.Button comparebtn;
		private System.Windows.Forms.Label resultlbl;
		private System.Windows.Forms.Label label1;
	}
}

